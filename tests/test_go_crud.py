...
def test_player_exists(godb, session, go_p1, go_p2):
    assert godb.player_exists(
        discord_id=go_p1.discord_id, session=session) == False
    assert godb.player_exists(
        discord_id=go_p2.discord_id, session=session) == False

    # Write Player to DB
    godb.create_player(go_player=go_p1, session=session)
    assert 1 == godb.player_count(session=session)
    assert godb.player_exists(
        discord_id=go_p1.discord_id, session=session) == True
    assert godb.player_exists(
        discord_id=go_p2.discord_id, session=session) == False
...
def test_delete_player(godb, session, go_p1, go_p2):
    ...
    # Read Player from DB
    assert None == godb.read_player(
        discord_id=go_p2.discord_id, session=session)
...
def test_create_and_read_team_and_roster(godb, session, go_p1, go_p2):
    ...
    team1 = godb.create_team(team_name="tn1", go_players=[
                             go_p1, go_p2], session=session)
    assert 2 == godb.player_count(session=session)
    ...
    team2 = godb.create_team(team_name="tn2", go_players=[
                             go_p1], session=session)
    assert 2 == godb.player_count(session=session)
...
def test_read_team_with_roster(godb, session, go_p1, go_p2):
    ...
    teama = godb.read_team_with_roster(
        discord_ids={go_p1.discord_id, go_p2.discord_id}, session=session)
    assert teama.id == team1.id

    teamb = godb.read_team_with_roster(
        discord_ids={go_p1.discord_id}, session=session)
    assert teamb.id == team2.id

    teamc = godb.read_team_with_roster(
        discord_ids={go_p2.discord_id}, session=session)
    assert teamc == None
...
def test_create_team_duplicate_roster(godb, session, go_p1, go_p2):
    ...
    with pytest.raises(GoDbError):
        teamx = godb.create_team(team_name="anything", go_players=[
                                 go_p1, go_p2], session=session)

    with pytest.raises(GoDbError):
        teamx = godb.create_team(team_name="anything else", go_players=[
                                 go_p1], session=session)
...
def test_read_player_signups_with_filters(godb, session, go_p1, go_p2, go_p3):
    ...
    team1 = godb.create_team(team_name="tn1", go_players=[
                             go_p1], session=session)
    team12 = godb.create_team(team_name="tn12", go_players=[
                              go_p1, go_p2], session=session)
    team123 = godb.create_team(team_name="tn123", go_players=[
                               go_p1, go_p2, go_p3], session=session)
    team23 = godb.create_team(team_name="tn23", go_players=[
                              go_p2, go_p3], session=session)
    team3 = godb.create_team(team_name="tn3", go_players=[
                             go_p3], session=session)
    ...
    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, date=date1)
    assert len(signups) == 3

    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, date=date1, team_id=team23.id)
    assert len(signups) == 2

    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, date=date6)
    assert len(signups) == 0

    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, team_id=team23.id)
    assert len(signups) == 4

    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, discord_id=go_p1.discord_id)
    assert len(signups) == 4

    signups: List[GoTeamPlayerSignup] = godb.read_player_signups(
        session=session, discord_id=0)
    assert len(signups) == 0
...
def test_set_session_date(godb, session):
    ...
    godb.set_session_date(session_id=channel_id1,
                          session_date=date1, session=session)
    ...
    godb.set_session_date(session_id=channel_id1,
                          session_date=date2, session=session)
...
def test_get_official_rating(godb, pfdb, session, pf_p1, go_p1):
    ...
    go_rating = godb.get_official_rating(
