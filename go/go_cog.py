...
class GoCog(commands.Cog):
    ...
    admin_group = app_commands.Group(
        name="goadmin", description="GO Admin Commands")
    ...
    def get_date_from_channel(self, channel_id: int, session: Session):
        date = self.godb.get_session_date(
            session_id=channel_id, session=session)
        if date is None:
    ...
    def set_rating_if_needed(self, pf_player_id, session) -> float:
        ...
        go_rating = self.godb.get_official_rating(
            pf_player_id=pf_player_id, session=session)

        if go_rating is None:
            go_rating = self.pfdb.calc_rating_from_stats(
                pf_player_id=pf_player_id, session=session)

            if go_rating is None:
                logger.error(
                    f"In get_rating_default -- calc_rating_from_stats failed for id {pf_player_id}")
                return None
            else:
    ...
    def do_set_ign(self, player: DiscordUser, ign: str, session: Session) -> GoPlayer:
        ...
        pf_p = None
        pf_players = self.pfdb.read_players_by_ign(
            ign=ign, session=session, limit=11)
    ...
    async def set_ign(self, interaction: discord.Interaction, ign: str):
        ...
        try:
            with Session(self.engine) as session:
                ...
                go_rating = self.set_rating_if_needed(
                    go_p.pf_player_id, session)
                if go_rating is None:
            ...
            logger.warn(
                f"set_ign resulted in error code {err.code}: {err.message}")
    ...
    def do_player_info(self, player: DiscordUser) -> str:
        with Session(self.engine) as session:
            ...
            if go_p is None:
                ...
                player_rating = self.godb.get_official_rating(
                    pf_player_id=go_p.pf_player_id, session=session)
                ...
                for team in teams:
                    ...
                    igns = sorted(
                        [r.player.pf_player.ign for r in team.rosters])
    ...
    async def player_info(self,
        ...
        try:
            ...
            logger.warn(
                f"player_info resulted in error code {err.code}: {err.message}")
    ...
    def do_rename_team(self, new_team_name: str, player: DiscordUser, date: datetype, session: Session) -> GoTeam:
        tpsignups = self.godb.read_player_signups(
            session=session, discord_id=player.id, date=date)
        if len(tpsignups) == 0:
    ...
    @go_group.command(
        description="Rename team for this channel's signup"
    )
    async def rename_team(self,
        ...
        if player is not None:
            if interaction.user.id != _config.owner_id:
                logger.warn(
                    f"User {get_name(interaction.user)} tried to run rename_team for another player")
                await interaction.response.send_message('You dont have permission to use this command for other players')
                return
        else:
            player = interaction.user

        p = convert_user(player)
        logger.info(
            f"Running rename_team({p.name}, {new_team_name}, {interaction.channel.name})")

        try:
            with Session(self.engine) as session:
                date = self.get_date_from_channel(
                    interaction.channel_id, session)
            ...
            logger.warn(
                f"rename_team resulted in error code {err.code}: {err.message}")
            await interaction.response.send_message(err.message)
    ...
    def do_signup(self, players: List[DiscordUser], team_name: str, date: datetype, session: Session) -> GoSignup:
        ...
        for player in players:
            ...
            go_player = self.godb.read_player(
                discord_id=player.id, session=session)
            ...
            player_rating = self.godb.get_official_rating(
                pf_player_id=go_player.pf_player_id, session=session)
        ...
        go_team_by_roster = self.godb.read_team_with_roster(
            discord_ids=discord_ids, session=session)
        ...
        if go_team_by_roster and go_team_by_roster.team_name != team_name:
            ...
            logger.info(
                f'In do_signup: team already has name "{team_name}".  Using that name instead.')

        # read_team_with_name can handle team_name=None
        go_team_by_name = self.godb.read_team_with_name(
        ...
        try:
            ...
            if go_team_by_roster is None:
                go_team_by_roster = self.godb.create_team(team_name=team_name, go_players=go_players,
                                                          session=session, rating_limit=rating_limit)
            ...
            signup = self.godb.add_signup(
                team=go_team_by_roster, date=date, session=session)
        ...
        try:
            ...
            with Session(self.engine) as session:
                ...
                signup = self.do_signup(
                    players=players, team_name=team_name, date=date, session=session)
            ...
            logger.warn(
                f"signup resulted in error code {err.code}: {err.message}")
            await interaction.response.send_message(err.message)
    ...
    def do_cancel(self, player: DiscordUser, date: datetype, session: Session) -> GoTeamPlayerSignup:
        ...
        if len(tpsignups) > 1:
            logger.error(
                f"Somehow player {player.name} has signed up more than once on {date}")
    ...
    async def cancel(self, interaction: discord.Interaction):
        ...
        logger.info("")
        logger.info(
            f"GoCog.cancel names ({interaction.channel}, {player.name})")
        logger.info(
            f"GoCog.cancel ids   ({interaction.channel_id}, {player.id})")
        ...
        try:
            with Session(self.engine) as session:
                ...
                tpsignup = self.do_cancel(
                    player=player, date=date, session=session)
            ...
            logger.warn(
                f"do_cancel resulted in error code {err.code}: {err.message}")
            await interaction.response.send_message(err.message)
    ...
    async def list_teams(
        ...
        logger.info(
            f"Running list_teams on  channel_id: {interaction.channel_id}  channel.name: {interaction.channel.name}")
        ...
        with Session(self.engine) as session:
            ...
            if date is None:
                ...
                teams = self.godb.get_teams_for_date(
                    session_date=date, session=session)
                msg = ''
                player_count = 0
                for team in teams:
    ...
    async def sync_commands(self, interaction: discord.Interaction):
        logger.info(
            f'Command sync called by user {get_name(interaction.user)}.')
        if interaction.user.id != _config.owner_id:
    ...
    async def wipe_commands(self, interaction: discord.Interaction):
        logger.info(
            f'Command clear_commands alled by user {get_name(interaction.user)}.')
        if interaction.user.id != _config.owner_id:
            logger.warn(
                f"User {get_name(interaction.user)} tried to run clear_commands")
            await interaction.response.send_message('You dont have permission to use this command!')
            return
    ...
    async def set_ign(
        ...
        if interaction.user.id != _config.owner_id:
            logger.warn(
                f"User {get_name(interaction.user)} tried to run set_ign")
            await interaction.response.send_message('You dont have permission to use this command!')
            return
    ...
    async def set_session_date(
        ...
        if interaction.user.id != _config.owner_id:
            logger.warn(
                f"User {get_name(interaction.user)} tried to run set_session_date")
            await interaction.response.send_message('You dont have permission to use this command!')
            return
        ...
        try:
            ...
            with Session(self.engine) as session:
                session_id = interaction.channel_id
                self.godb.set_session_date(
                    session_id=session_id, session_date=date2, session=session)
    ...
    async def get_session_date(
        ...
        if interaction.user.id != _config.owner_id:
            logger.warn(
                f"User {get_name(interaction.user)} tried to run get_session_date")
            await interaction.response.send_message('You dont have permission to use this command!')
            return
