...
def main():
    ...
    parser.add_argument("--statname", default='CareerWins', type=str, required=False,
                        help="CareerWins, CareerKills, CareerDamage, WeeklyWinsTotal, WeeklyKillsTotal")
    parser.add_argument("--min", default=0, type=int, required=False,
                        help="Stop running after statname value gets below min")
    parser.add_argument("--recent", default=1, type=float, required=False,
                        help="Skip getting new stats if there are recent ones within x days")
    ...
    with Session(engine) as session:
        ...
        while start < end:
            ...
            leaderboard = pfapi.get_leaderboard(
                start_rank=start, batchsize=batchsize, stat_name=args.statname)
            start += batchsize
            ...
            for lb_row in leaderboard:
                ...
                if args.min and lb_row.stat_value < args.min:
                    logger.info(
                        f"Minimum stat value reached {lb_row.stat_value} < {args.min}")
                    start = end
                    break
                ...
                logger.info(
                    f"Rank {lb_row.stat_rank}, Value {lb_row.stat_value} -- {player}")
                ...
                player = pfdb.read_player(
                    pf_player_id=player.id, session=session)
                ...
                if career_stats and (now - career_stats[-1].date) < timedelta(days=args.recent):
                    ...
                    logger.info(
                        f"Already have recent stats for: {player.ign} at {career_stats[-1].date}")
                    pass
                else:
                    logger.info(f"Getting stats for: {player.ign}")
                    try:
                        stats = pfapi.get_player_career_stats(
                            player_id=player.id)
                        session.add(stats)
                        session.commit()
